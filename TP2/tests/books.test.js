...
test('POST /books ajoute un nouveau livre', async () => {
    const newBook = { id: 3, title: 'Book C', author:
            'Author C' };
...
test('DELETE /books/:id supprime un livre', async () => {
    ...
    const booksBeforeDeletion = await request(app).get('/books');
    ...
    expect(booksAfterDeletion.body.length).toBe(booksBeforeDeletion.body.length - 1);
});